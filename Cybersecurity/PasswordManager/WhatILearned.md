...
I ran into a problem the moment I started thinking about writing user credentials to my credentials.txt file. I store user credentials like so: 

method of encryption, username, encrypted password, (any other relevant data given method of encryption)

For example, if the user had a username "JohnDoe123" and a password "Password321" and chose XOR, the saved credentials in credentials.txt would store the method (XOR), username, encrypted password, and the key, which is used during the sign in process. In credentials.txt, it would look like this:

XOR, JohnDoe123, o♠å7[►╖╙^J♫, ?g⌡D,⌂┼╖mx

The problem is that the credentials need to be separated with commas in the text file, but commas can easily appear as part of encrypted passwords. My first plan was to hard-code commas out of my encryption functions, and I went to ChatGPT and gave it my plan and pseudo code and asked it if my idea was a valid solution to my problem. It politely said "no, that's actually very incorrect" and introduced me to the concept of encoding, specifically Base64. It's an incredibly neat process: Base64 maps binary values to "safe" characters contained in the string: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", it does this by taking the 8-bit binary representation of each character in the input string and concatenating them into one string of bits. It then goes through said string of bits in intervals of six bits at a time, mapping the decimal value of each 6-bit block to the corresponding index in the string of safe characters. So if the 6-bit block was "001001" (9), it would grab the character at the ninth index in the safe character string (J) and add that character to the output/encoded string. In this sense, we treat any input string as just a continuous string of bits. If we go through this string in 8-bit chunks and conccatenated the corresponding ASCII character at that 8-bit decimal value, we'd get the original string. If we went through the bit string 6-bits at a time (after padding the bit string with zeros, if it's not at first evenly divisible by six,) took each 6-bit decimal value and grabbed the character at the corresponding index of the Base64 safe character string, 